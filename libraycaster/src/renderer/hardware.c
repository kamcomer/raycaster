// #include "internal/renderer/gpu_renderer_int.h"
...
// static SDL_GPUShader *compile_shader_from_hlsl_source(RcGPURenderer *r, const char *source,
//                                                       SDL_ShaderCross_ShaderStage stage,
//                                                       const char *entrypoint)
// {
//   SDL_ShaderCross_HLSL_Info hlsl_info = {.source = source,
...
//   SDL_ShaderCross_SPIRV_Info spirv_info = {.bytecode = spirv,
...
// {
//   r->wall_vertex_shader = compile_shader_from_hlsl_source(
//       r, g_wall_hlsl, SDL_SHADERCROSS_SHADERSTAGE_VERTEX, "vertex_main");
//   if (!r->wall_vertex_shader)
//     return false;

//   r->wall_fragment_shader = compile_shader_from_hlsl_source(
//       r, g_wall_hlsl, SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "fragment_main");
...
//   r->sprite_vertex_shader = compile_shader_from_hlsl_source(
//       r, g_sprite_hlsl, SDL_SHADERCROSS_SHADERSTAGE_VERTEX, "vertex_main");
//   if (!r->sprite_vertex_shader)
//     return false;

//   r->sprite_fragment_shader = compile_shader_from_hlsl_source(
//       r, g_sprite_hlsl, SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "fragment_main");
...
//   r->floor_vertex_shader = compile_shader_from_hlsl_source(
//       r, g_floor_hlsl, SDL_SHADERCROSS_SHADERSTAGE_VERTEX, "vertex_main");
//   if (!r->floor_vertex_shader)
//     return false;

//   r->floor_fragment_shader = compile_shader_from_hlsl_source(
//       r, g_floor_hlsl, SDL_SHADERCROSS_SHADERSTAGE_FRAGMENT, "fragment_main");
...
// static bool upload_texture_layer(RcGPURenderer *r, SDL_GPUTexture *texture, uint32_t *pixels,
//                                  int width, int height, int layer)
// {
//   SDL_GPUTransferBufferCreateInfo transfer_info = {.size = width * height * 4,
...
//       .texture = texture, .w = (Uint32)width, .h = (Uint32)height, .d = 1, .layer = layer};
...
// static SDL_GPUTexture *create_texture_2d_array(RcGPURenderer *r, int width, int height, int
// layers)
// {
//   SDL_GPUTextureCreateInfo tex_desc = {.type = SDL_GPU_TEXTURETYPE_2D_ARRAY,
...
// {
//   SDL_GPUTextureCreateInfo tex_desc = {.type = SDL_GPU_TEXTURETYPE_2D,
...
//   // TODO: Calculate frame arena size based on screen resolution
//   r->transfer_buffer_size = 512 * 1024;
//   SDL_GPUTransferBufferCreateInfo info = {.size = r->transfer_buffer_size,
//                                           .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD};
//   r->transfer_buffer = SDL_CreateGPUTransferBuffer(r->device, &info);
...
//   r->wall_texture_array =
//       create_texture_2d_array(r, TEXTURE_WIDTH, TEXTURE_HEIGHT, MAX_WALL_TEXTURES);
//   if (!r->wall_texture_array) {
...
//     if (!upload_texture_layer(r, r->wall_texture_array, textures[i], TEXTURE_WIDTH,
//     TEXTURE_HEIGHT,
//                               i)) {
...
// int gpu_renderer_set_floor_texture(RcGPURenderer *r, uint32_t *pixels)
// {
...
// int gpu_renderer_set_ceiling_texture(RcGPURenderer *r, uint32_t *pixels)
// {
//   if (!r || !r->device || !pixels)
//     return -1;
...
//   if (!upload_texture_layer(r, r->sprite_textures[index], pixels, TEXTURE_WIDTH, TEXTURE_HEIGHT,
//                             0)) {
...
//     if (r->wall_vertex_buffer)
//       SDL_ReleaseGPUBuffer(r->device, r->wall_vertex_buffer);
//     SDL_GPUBufferCreateInfo buffer_info = {.size = wall_vertex_size,
...
//     if (r->wall_index_buffer)
//       SDL_ReleaseGPUBuffer(r->device, r->wall_index_buffer);
//     SDL_GPUBufferCreateInfo buffer_info = {.size = wall_index_size,
...
//     if (r->floor_vertex_buffer)
//       SDL_ReleaseGPUBuffer(r->device, r->floor_vertex_buffer);
//     SDL_GPUBufferCreateInfo buffer_info = {.size = floor_vertex_size,
//                                            .usage = SDL_GPU_BUFFERUSAGE_VERTEX};
...
// static void upload_buffer_data(RcGPURenderer *r, SDL_GPUBuffer *buffer, const void *data,
//                                size_t size)
// {
//   SDL_GPUTransferBufferCreateInfo transfer_info = {.size = size,
//                                                    .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD};
//   SDL_GPUTransferBuffer *transfer = SDL_CreateGPUTransferBuffer(r->device, &transfer_info);
//   if (!transfer)
//     return;
...
//   r->transfer_mapped = SDL_MapGPUTransferBuffer(r->device, r->transfer_buffer, false);
//   r->transfer_offset = 0;
//   r->frame_begun = true;
// }
...
// void gpu_renderer_draw_scene(RcGPURenderer *r, float cam_x, float cam_y, float dir_x, float
// dir_y,
//                              float plane_x, float plane_y, int screen_width, int screen_height,
//                              int *map, int map_width, int map_height, void *intersects_ptr,
//                              int num_intersects, void *sprites_ptr, int num_sprites)
// {
//   (void)cam_x;
//   (void)cam_y;
//   (void)dir_x;
...
//     WallVertex *vertices = (WallVertex *)(r->transfer_mapped + r->transfer_offset);
//     r->transfer_offset += vertex_data_size;

//     uint16_t *indices = (uint16_t *)(r->transfer_mapped + r->transfer_offset);
//     r->transfer_offset += index_data_size;

//     if (r->transfer_offset > r->transfer_buffer_size)
//       return;
...
//       SDL_GPUCopyPass *copy_pass = SDL_BeginGPUCopyPass(r->cmd_buffer);
//       SDL_GPUTransferBufferLocation src = {.transfer_buffer = r->transfer_buffer};

//       size_t vert_bytes = num_vertices_to_upload * sizeof(WallVertex);
//       src.offset = 0;
//       SDL_GPUBufferRegion dst = {
//           .buffer = r->wall_vertex_buffer, .offset = 0, .size = (uint32_t)vert_bytes};
//       SDL_UploadToGPUBuffer(copy_pass, &src, &dst, false);

//       size_t idx_bytes = num_indices_to_upload * sizeof(uint16_t);
//       src.offset = vert_bytes;
//       dst = (SDL_GPUBufferRegion){
//           .buffer = r->wall_index_buffer, .offset = 0, .size = (uint32_t)idx_bytes};
//       SDL_UploadToGPUBuffer(copy_pass, &src, &dst, false);

//       src.offset = floor_offset;
//       dst = (SDL_GPUBufferRegion){
//           .buffer = r->floor_vertex_buffer, .offset = 0, .size = sizeof(floor_vertices_data)};
//       SDL_UploadToGPUBuffer(copy_pass, &src, &dst, false);
...
//   SDL_GPUColorTargetInfo color_target = {.texture = r->swapchain_texture,
//                                          .load_op = SDL_GPU_LOADOP_CLEAR,
//                                          .store_op = SDL_GPU_STOREOP_STORE,
//                                          .clear_color = {0.1f, 0.1f, 0.15f, 1.0f}};

//   SDL_GPUDepthStencilTargetInfo depth_target = {.texture = r->depth_texture,
//                                                 .load_op = SDL_GPU_LOADOP_CLEAR,
//                                                 .store_op = SDL_GPU_STOREOP_STORE,
//                                                 .clear_depth = 1.0f};

//   SDL_GPURenderPass *render_pass =
//       SDL_BeginGPURenderPass(r->cmd_buffer, &color_target, 1, &depth_target);
//   if (!render_pass) {
...
//     if (r->wall_texture_array) {
//       SDL_GPUTextureSamplerBinding texture_binding = {.texture = r->wall_texture_array,
//                                                       .sampler = r->sampler};
