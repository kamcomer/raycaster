...
static void render_floor_ceiling(RcEngine *e)
{
    ...
    for (int y = h / 2; y < h; ++y)
    {
        ...
        if (rowDistance < 0.1f) rowDistance = 0.1f;
...
static void render_sprites(RcEngine *e, double *z_buffer)
{
    ...
    for (int i = 0; i < sprite_count; i++)
    {
        ...
        {
            int tex_x = (int)((stripe - (-sprite_width / 2 + sprite_screen_x)) * RC_TEXTURE_WIDTH / sprite_width);
...
static void render_walls(RcEngine *e, double *z_buffer)
{
    ...
    for (int x = 0; x < w; x++)
    {
        ...
        if (side == 0 && ray_dir_x > 0) tex_x = RC_TEXTURE_WIDTH - tex_x - 1;
        if (side == 1 && ray_dir_y < 0) tex_x = RC_TEXTURE_WIDTH - tex_x - 1;
