...
int rc_gpu_renderer_upload_textures(RcGPURenderer *r, uint32_t *textures[], int count)
{
  ...
  for (int i = 0; i < upload_count; i++) {
    if (!upload_texture_layer(r, r->wall_texture_array, textures[i], TEXTURE_WIDTH, TEXTURE_HEIGHT,
                              i)) {
...
void rc_gpu_renderer_draw_scene(RcGPURenderer *r, float cam_x, float cam_y, float dir_x,
                                ...
                                void *intersects_ptr, int num_intersects, void *sprites_ptr,
                                int num_sprites)
{
  (void)cam_x;
  (void)cam_y;
  ...
  if (num_intersects > 0) {
    ...
    if (num_vertices_to_upload > 0) {
      upload_buffer_data(r, r->wall_vertex_buffer, vertices,
                         num_vertices_to_upload * sizeof(WallVertex));
      upload_buffer_data(r, r->wall_index_buffer, indices,
                         num_indices_to_upload * sizeof(uint16_t));
    }
