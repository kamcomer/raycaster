...
void update_player(Player *player, float delta_time, Map map)
{
  ...
  rotate_vector(&player->plane, player->actor->dir.angle - player->plane.angle + PI_2);
...
void cast_player_rays(Player *player, Map map)
{
  ...
  Vector pos =
      set_vector(player->actor->pos.x / map.unit_size, player->actor->pos.y / map.unit_size);

  double rad_per_col = player->actor->field_of_view / win_config->width; // Radians per pixel
  ...
  for (int x = 0; x < win_config->width; x++) {
    ...
    camera_x = 2 * x / (double)win_config->width - 1; // Normalized coordinate in camera space
    Vector ray_dir = set_vector(dir.x + plane.x * camera_x, dir.y + plane.y * camera_x);
...
WallIntersect get_wall_intersect(Vector origin, Vector ray_dir, Map map)
{
  ...
  intersect.vect =
      set_vector(origin.x + ray_dir.x * perp_wall_dist, origin.y + ray_dir.y * perp_wall_dist);
...
Player create_player(WindowCtx *window_ctx)
{
  ...
  player.plane = set_vector(0, player.actor->dir.x * tan(player.actor->field_of_view / 2));
